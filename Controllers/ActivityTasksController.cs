// -----------------------------------------------------------------------
// <copyright file="ActivityTasksController.cs" company="Nodine Legal, LLC">
    ...
    public class ActivityTasksController : BaseController
    {
        ...
        public ActionResult Create(long? regleadid, long? regopportunityid)
        {
            ...
            using (IDbConnection conn = Data.Database.Instance.GetConnection())
            {
                ...
                viewModel.Type = Mapper.Map<ViewModels.Activities.ActivityTypeViewModel>(Data.Activities.ActivityType.Get("Task", conn, false));
            }
        ...
        [HttpPost]
        [Authorize(Roles = "Login, User")]
        public ActionResult Create(ViewModels.Activities.ActivityTaskViewModel viewModel)
        {
            Common.Models.Account.Users currentUser;
            ...
            using (Data.Transaction trans = Data.Transaction.Create(true))
            {
                try
                {
                    ...
                    model = Mapper.Map<Common.Models.Activities.ActivityTask>(viewModel);
                    model.Type = Data.Activities.ActivityType.Get(trans, "Task");

                    if (!model.IsCampaignResponse.HasValue)
                        model.IsCampaignResponse = false;                    

                    model = Data.Activities.ActivityTask.Create(trans, model, currentUser);
        ...
        [HttpPost]
        [Authorize(Roles = "Login, User")]
        public ActionResult Edit(long id, ViewModels.Activities.ActivityTaskViewModel viewModel)
        {
            Common.Models.Account.Users currentUser;
            Common.Models.Activities.ActivityTask model;
            ...
            using (Data.Transaction trans = Data.Transaction.Create(true))
            {
                try
                {
                    ...
                    model = Data.Activities.ActivityTask.Edit(trans, model, currentUser);
                    trans.Commit();
